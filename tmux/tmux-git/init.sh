...
CONFIG_FILE="$(cd $(dirname ${BASH_SOURCE[0]}); pwd)/config"
...
update_tmux() {
    ...
    if [[ $TMUX_GIT_LASTREPO ]] && [ "$TMUX_GIT_LASTREPO" = "${CWD:0:$LASTREPO_LEN}" ]; then
        ...
        if [ "$GIT_DIRTY" = '*' ]; then 
            # tmux set-window-option status-$TMUX_STATUS_LOCATION-attr bold > /dev/null
            TMUX_STATUS="$(echo "$TMUX_STATUS" | sed 's/=green/=red/' | sed 's/=blue/=brightred/')"
        else
            # tmux set-window-option status-$TMUX_STATUS_LOCATION-attr none > /dev/null
            TMUX_STATUS="$(echo "$TMUX_STATUS" | sed 's/=red/=green/' | sed 's/=brightred/=blue/')"
        fi
